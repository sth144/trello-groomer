...
export function detectRemovals(
    newObject: ConfigObj, oldObject: ConfigObj, pathSoFar: string[] = []
): string[][] {
    let dotPaths: string[][] = [];
    Object.keys(oldObject).forEach((k) => {
        if (newObject.hasOwnProperty(k)) {
            ...
            if (isArray(oldObject[k])) {
                ...
                dotPaths = dotPaths.concat(detectRemovals(
                    newObject[k] as ConfigObj, oldObject[k] as ConfigObj, 
                        [...pathSoFar, k]));
            }
        } else {
                    ...
                    case "object": {
                        if (!isArray(newObject[k])) {
                            updates = updates.concat(
                                detectLiteralChanges(newObject[k] as ConfigObj, 
...
export function updateLiteralsByDotPath(
    target: ConfigObj, updates: LiteralUpdate[]): void {
    updates.forEach((update) => {
        if (target.hasOwnProperty(update.dotPath[0])) {
            if (typeof target[update.dotPath[0]] === "object" 
                && !isArray(target[update.dotPath[0]])) {
                updateLiteralsByDotPath(target[update.dotPath[0]] as ConfigObj, 
                    [update].map(x => {
...
export function removePropsByDotPath(target: ConfigObj, dotPaths: string[][]): void {
    const arraysAtRootLevel: { [key: string]: any[] } = { };
    dotPaths.forEach((path) => {
        if (target.hasOwnProperty(path[0])) {
            if (isArray(target[path[0]])) {
                if (path.length > 1) {
                    const targetArray = (<Array<number | string>>target[path[0]]).slice(0);
                    ...
                    arraysAtRootLevel[path[0]] = targetArray.filter((val) => val.toString() !== valueToRemove);
                } else {
...
export function syncObjectsWithPreference(preferred: ConfigObj, secondary: ConfigObj): ConfigObj {
    const result = { };
    ...
    Object.keys(preferred).forEach((k) => {
        ...
        /** if property is array */
            ...
            if (secondary.hasOwnProperty(k)) {
                [(preferred[k] as Array<ConfigPropType>), (secondary[k] as Array<ConfigPropType>)]
                    .forEach((array) => {
                        array.forEach((x) => {
            ...
            if (secondary.hasOwnProperty(k)) {
                resultPropObj = syncObjectsWithPreference(preferred[k] as ConfigObj, secondary[k] as ConfigObj);
            } 
            Object.assign(result, {
                [k]: resultPropObj
            });
