  ...
  Object.entries(autoDueConfigRaw).forEach((x: [string, unknown]) => {
    if (typeof x[1] === 'object') {
      if (x[1].hasOwnProperty('periodType')) {
        if (!x[1].hasOwnProperty('endOfPeriod'))
          Object.assign(x[1], { endOfPeriod: true });
        if (!x[1].hasOwnProperty('dividePeriodBy'))
          Object.assign(x[1], { dividePeriodBy: 1 });
        if (!x[1].hasOwnProperty('multiplyPeriodBy'))
          Object.assign(x[1], { multiplyPeriodBy: 1 });
        ...
        numDays = Math.round(
          ((x[1] as { multiplyPeriodBy: number }).multiplyPeriodBy *
            numerator) /
