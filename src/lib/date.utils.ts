...
export const DateRegexes = {
  ...
  /** ex. Feb3@16:20 */
  MonthDayTime: new RegExp(
    /(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[0-9]{1,2}@[0-9]{1,2}(:[0-9]{1,2})?((a|A|p|P)(m|M))?/
  ),
  /** ex. 4:20pm */
  TimeNonMil: new RegExp(/[0-9]{1,2}(:[0-9]{1,2})?(a|A|p|P)(m|M)/),
  /** day name and time, ex. Mon@13:30 */
  DayNameTime: new RegExp(
    /(Mon|Tue|Wed|Thu|Fri|Sat|Sun)@[0-9]{1,2}(:[0-9]{1,2})?((a|A|p|P)(m|M))?/
  ),
  /** ex. January 2020 */
  MonthYear: new RegExp(
    /(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec){1}[a-z]{0,6} [0-9]{4}/
  ),
};
...
export function parseDueDate(
  ...
  if ((extractDue = inputStr.match(DateRegexes.DateTimeStr))) {
    ...
    if ((extractDue = inputStr.match(DateRegexes.MonthDayTime))) {
      ...
      if (
        extractDayName !== null &&
        extractDayName.length > 0 &&
        extractDayName[0] !== null
      ) {
