...
import { 
    cardIsComplete, cardDueToday, cardDueThisWeek, 
    cardHasDueDate, wasMovedFromToListFilterFactory, Not 
...
export const WorkGroomer = function() {
    ...
    const groom = async () => {
        ...
        if (existsSync(autoDueConfigPath)) {
            ...
            await workController.assignDueDatesIf(workModel.lists.week.id, autoDueConfig.week,
                wasMovedFromToListFilterFactory(workModel.lists.week.id, [
                    workModel.lists.day.id
                ]));

            await workController.assignDueDatesIf(workModel.lists.day.id, autoDueConfig.day, 
                Not(cardHasDueDate));
            await workController.assignDueDatesIf(workModel.lists.week.id, autoDueConfig.week,
                Not(cardHasDueDate)); 
            await workController.assignDueDatesIf(workModel.lists.backlog.id, autoDueConfig.backlog, 
        ...
        await workController.moveCardsFromToIf([
            ...
            workModel.lists.week.id,
            workModel.lists.day.id
        ], workModel.lists.done.id, cardIsComplete);
        ...
        await workController.moveCardsFromToIf([
            ...
            workModel.lists.backlog.id,
            workModel.lists.week.id,
        ], workModel.lists.day.id, cardDueToday);

        /** move cards due this week to Week */
        await workController.moveCardsFromToIf([
            workModel.lists.inbox.id,
            workModel.lists.backlog.id,
        ], workModel.lists.week.id, cardDueThisWeek);

        /** move all cards in inbox with due date to backlog */
        await workController.moveCardsFromToIf([
            workModel.lists.inbox.id
        ], workModel.lists.backlog.id, cardHasDueDate);
