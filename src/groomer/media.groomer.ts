...
export const MediaGroomer = function() {
    ...
    const groom = async () => {
        ...
        const subprocess = spawn("python3", ["get_library.py"], { cwd: "./py/audible" });
        ...
        if (existsSync(audibleLibraryOutputPath)) {
            ...
            const allCardTitlesOnBoard = mediaModel.getAllCardNames().map(x => x.toLowerCase());

            const bookLabel = mediaModel.getLabels()["Books"];
            const inboxListId = 
                ( <Record<MediaBoardLists, { id: string }>>
                    mediaModel.getLists() )[MediaBoardLists.inbox].id;

            [ audibleLibraryInfo.library, audibleLibraryInfo.wishlist ]
                .forEach((collection) => {
                    collection.forEach(async (item: string) => {
                        ...
                        if (
                            !allCardTitlesOnBoard.some(title => title.indexOf(itemLowerCase) !== -1)
                         && !allCardTitlesOnBoard.some(title => itemLowerCase.indexOf(title) !== -1)
                         && allCardTitlesOnBoard.indexOf(itemLowerCase) === -1) {
