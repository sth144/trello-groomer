...
export class BoardController<T extends BoardModel> {
  ...
  public hasLabelFilterFactory(labelName: string) {
    ...
    return (card: ICard) => {
      if (
        card.idLabels.filter((idLabel) => idLabel === targetLabelId).length > 0
      )
        return true;
      return false;
    };
  ...
  public async updateTaskDependencies(
        ...
        for (const checklistItem of checklists[checklistId].checkItems) {
          ...
          if (
            ...
            await this.httpClient.asyncDelete(
              `/checklists/${checklistId}/checkItems/${checklistItem.id}/`
            );
            const replacedCheckItem = await this.httpClient.asyncPost(
            ...
            await this.httpClient.asyncPost(
              `/cards/${childCard.id}/attachments`,
              {
     ...
     */
    for (const card of this.boardModel
      .getAllCards()
      .filter((x) => !ignoreLists.some((l) => l.id === x.idList))) {
        ...
        for (const attachment of card.attachments) {
          ...
          if (attachment.name.indexOf("parent") !== -1) {
            ...
            if (
              ...
              this.httpClient
                .asyncPut(
                  `/cards/${parsed.parent}/checkItem/${parsed.checkItemId}?state=complete`
                )
                .catch((err) => {
                  logger.info(err);
                });
   ...
   * TODO: document rules
   */
  public async updatePrepDependencies(
        ...
        targetChecklist.checkItems.map((checklistItem) => {
          allCards.map(async (prepCard) => {
            ...
            if (checklistItem.name === prepCard.name) {
              ...
              const dependentCard = this.boardModel.getCardById(
                checklists[checklistId].idCard
              );
              /** provide link to dependent card in prep card */
              await this.httpClient.asyncPost(
                `/cards/${prepCard.id}/attachments`,
                {
    ...
    if (targetChecklist !== null) {
      ...
      allCards
        .filter((card) => card.dueComplete && card.badges.attachments > 0)
          ...
          card.attachments.map((attachment: any) => {
            if (
              ...
              if (
                ...
                this.httpClient
                  .asyncPut(
                    `/cards/${parsed.dependent}/checkItem/${parsed.checkItemId}?` +
                      `state=complete`
                  )
                  .catch((err) => {
                    logger.info(err);
  ...
  public async updateFollowupDependencies(
    targetChecklistName: string,
    ignoreLists: List[] = []
  ): Promise<void> {
    const checklists = this.boardModel.getChecklists();
    ...
    Object.keys(checklists)
      .filter(
        (checklistId) => checklists[checklistId].name === targetChecklistName
      )
      .forEach((checklistId) => {
        targetChecklist = checklists[checklistId];
      ...
      if (checklistItem.state === "complete") {
        ...
        if (splitCheckItemName.length > 1) {
          for (const card of this.boardModel.getAllCards()) {
            if (
              !card.dueComplete &&
              checklistItem.name.indexOf(card.shortUrl) !== -1
            ) {
              await this.httpClient.asyncPut(
                `/cards/${card.id}?dueComplete=true`
              );
            }
          }
        }
   ...
   *                  be moved
   */
  public async moveCardsFromToIf(
    fromListIds: string[],
    toListId: string,
    ...
    for (const fromListId of fromListIds) {
      ...
      for (const card of fromListCards) {
        if (
          filter(card) &&
          card.idList === fromListId /** this double check may prevent a bug */
        ) {
          // TODO: this should be encapsulated in a moveCard operation
          await this.httpClient.asyncPut(
            `/cards/${card.id}?idList=${toListId}&pos=top`
          );

          /** update local model */
          [fromListId, toListId].forEach(async (id) => {
            this.boardModel.getListById(id).cards =
              await this.httpClient.asyncGet(`/lists/${id}/cards`);
          });
  ...
  public async assignDueDatesIf(
    listId: string,
    dueInDays: number,
    conditionFilter: (card: ICard) => boolean,
    randomStagger: number = null
  ): Promise<void> {
    const createDueDate = () => {
      ...
      if (conditionFilter(targetListCards[i])) {
        ...
        if (i > 0 && i < targetListCards.length - 1) {
          ...
          if (
             ...
             */
            newDueDate = getMidPointBetweenDates(
              new Date(cardAbove.due),
        ...
        batch.push(
          this.httpClient.asyncPut(`/cards/${card.id}?due=${newDueDate}`)
        );
  ...
  public async addLabelToCardsInListIfTitleContains(
    ...
    if (
      allLabels.hasOwnProperty(labelName) &&
      allLabels[labelName].length > 0
    ) {
  ...
  public async deleteCardsInListIfLabeled(
    listId: string,
    labelName: string
  ): Promise<void> {
    this.boardModel
  ...
  public async autoLinkRelatedCards(ignorePatterns: string[]): Promise<void> {
    ...
    Object.keys(labelAssociations).forEach(async (key) => {
      for (let cardA of labelAssociations[key]) {
        ...
        for (let cardB of labelAssociations[key].filter(
          (x) => x.name !== cardA.name
        )) {
        ...
        const numPreExistingTrelloAttachments =
          cardA.badges.attachmentsByType.trello.card;
        /**
         * if for each card for which titles share a common word > 3 characters
         */
        if (
          potentialLinksForCardA.length > 0 &&
          numPreExistingTrelloAttachments < maxTrelloAttachments
        ) {
          const numAttachmentsCanAdd =
            maxTrelloAttachments - numPreExistingTrelloAttachments;
          ...
          potentialLinksForCardA.forEach((cardToLink: ICard) => {
            ...
            if (
              !cardA.attachments.some((x: unknown): boolean => {
                return (
                  (x as { url: string }).url.indexOf(cardToLink.shortUrl) !== -1
                );
              })
            ) {
  ...
  public async parseDueDatesFromCardNames(): Promise<void> {
    for (const card of this.boardModel.getAllCards()) {
      ...
      if (
        ...
        await this.httpClient.asyncPut(
          `/cards/${card.id}?due=${dueDate}&name=${parsedName}`
        );
      }
    }
  }
  ...
  public async syncConfigJsonWithCard(jsonFileName: string, cardName: string) {
    ...
    updateLiteralsByDotPath(
      updateFromCard,
      detectLiteralChanges(
        loadedConfig as ConfigObj,
        prevConfigUpdate as ConfigObj
      )
    );

    removePropsByDotPath(
      loadedConfig,
      detectRemovals(updateFromCard, prevConfigUpdate)
    );
    removePropsByDotPath(
      updateFromCard,
      detectRemovals(loadedConfig, prevConfigUpdate)
    );

    const configUpdate = syncObjectsWithPreference(
      updateFromCard,
      loadedConfig
    );

    await this.httpClient.asyncPut(
      `/cards/${targetConfigSyncCard.id}?desc=${JSON.stringify(configUpdate)}`
    );
  ...
  private async buildModel(): Promise<void> {
      ...
      for (const listNameToFetch of this.boardModel.getListNames()) {
        if (
          ...
          modelListsHandle[listNameToFetch].cards =
            await this.httpClient.asyncGet(
              `/lists/${responseList.id}/cards` +
                `?attachments=true` +
     ...
     */
    const checklistsOnBoard = await this.httpClient.asyncGet(
      `/boards/${this.boardModel.id}/checklists`
    );
      ...
      if (
        ...
        if (responseChecklist) {
          ...
          for (const responseCheckItem of responseChecklist.checkItems) {
            ...
            this.boardModel
              .getChecklists()
              [responseChecklist.id].checkItems.push(newCheckItem);
          }
        }
      }
    }
    ...
    for (const responseList of this.allListsOnBoard) {
      ...
      if (qualifies && !modelListsHandle.hasOwnProperty(responseList.name)) {
        ...
        modelListsHandle[responseList.name].cards =
          await this.httpClient.asyncGet(
            `/lists/${responseList.id}/cards?attachments=true&actions=deleteAttachmentFromCard,updateCard`
          );
        result.push(newList);
      }
    }
  ...
  public importListsFromController(
    fromController: BoardController<BoardModel>
  ): List[] {
    const result: List[] = [];
    const fromControllerLists: Record<string, List> = <Record<string, List>>(
      fromController.boardModel.getLists()
    );
    for (const listIdx in fromControllerLists) {
      const fromList = fromControllerLists[listIdx];
      const copiedListWithTranslatedLabels: List = Object.assign(
        new List(),
        fromList
      );

      fromList.cards.forEach((x) => {
        x.idLabels = this.translateLabels(
          x.idLabels,
          fromController.boardModel.getLabels()
        );
      });
    ...
    result.forEach((resultList) => {
      this.boardModel.addList(resultList.name, resultList);
    });
    return result;
  }
  ...
  private translateLabels(
    inputLabels: string[],
    inputLabelDict: Record<string, string>
  ): string[] {
    const result: string[] = [];
  ...
  public dump(): void {
    ...
    writeFileSync(
      join(process.cwd(), "cache/labels.json"),
    ...
    writeFileSync(
      join(process.cwd(), "cache/label-data.json"),
    ...
    const unlabeledCards: any[] = this.boardModel
      ...
      .map((x) => x.name);
    writeFileSync(
      join(process.cwd(), "cache/unlabeled.json"),
      JSON.stringify(unlabeledCards)
    );

    writeFileSync(
      join(process.cwd(), "cache/model.json"),
